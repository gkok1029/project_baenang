...
<body>
	...
	<header>
		<div id="topcenprowrap">
			<div id=topcenpro>
				<!-- 파일 업로드를 위한 input 요소 -->
				<form id="imgF" name="imgF" action="mypageinfopic" method="post"
					enctype="multipart/form-data">
					<input type="file" id="editmf" name="editmf" style="display: none"
						accept="image/*">
				</form>
				<p class="profile_img">
					<span class="cursor"> <img id="proimg"
						alt="${user.getM_NNAME()}"
						src="/resources/profile/${user.getM_IMAGE()}"> <img
						id="edit" alt="edit" src="../resources/img/profile/edit.png"
						name="edit" onclick="editPic()">
					</span>
				</p>
				<p class="username">
	...
	<main>
		<div id="wrap">
			<div id="innerwrap">
				<div id="formContainer">
					<form action="mypageinfosubmit" name="memberinfo" class="info"
						id="infoF" method="post">
						<h3 class="pad">닉네임</h3>
						<div id="inputContainer" class="pad">
							<input type="text" name="nick" value="${user.getM_NNAME()}">
						</div>
						<h3 class="pad">이메일</h3>
						<div id="inputContainer" class="pad">
							<input type="text" name="userid" value="${user.getM_EMAIL()}"
								readonly>
						</div>
					</form>
		...
		<script type="text/javascript">
			function openPwd() {
				...
				win = open(url, "myPagepwdChange",
						"width=800, height=300, left=250, top=250")
			}

			function openOut() {
				let url = "../user/mypageout";
				win = open(url, "myPageOut",
						"width=400, height=400, left=250, top=250")
			}
